        ...
        try	{
            ...
            if (processThisFrame)
            {
                ...
                if (*frame_counter == 0)
                {
                    ...
                    me->ref_coords = currentFrame.coordinates.col(0);
                    //this is the rmsf reference frame
                    //perform gathering
                    ...
                    me->reference_coordinates = currentFrame.coordinates.colwise() - currentFrame.solute_cog;

                    //skip frame[0] from the reference process, because it is the first frame 
                    //and has no reference coordinates are available for the first atom
                    //also get the time of the first frame as reference for time-based skipping
                    //also hard-code using first atom in trajectory as reference which
                    //can be changed later by modifying the code
                   // me->ref_coords = me->reference_coordinates.col(0);
                    //me->ref_coords = currentFrame.coordinates.col(0);
                ...
                else
                {
                    ...
                    Gather::FirstAtomBasedBoxShifter(&currentFrame, 0, me, &grids->firstAtomBasedBoxShifter[0]);
