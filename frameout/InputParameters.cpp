...
void InputParameters::PrintGenericParameters(Structs::GenericParameters &me, gz::ogzstream &outfile) {
    //printing out input parameters in the trajectory file. this i a temporary solution. 
    //this should be printed in a separated and real log file.
	outfile << "REMARK    1 " <<  "#frameout: input parameters defined by user" << std::endl;
	outfile << "REMARK    1 " <<  "#--------------------------------------------------------------------------------" << std::endl;
	outfile << "REMARK    1 " <<  std::left;
	outfile << "REMARK    1 " <<  "#   number of atom records          : " << me.atomrecords << std::endl;
	//outfile << "REMARK    1 " <<  "#   distance cut-off                : " << me.distance_cut_off << std::endl;
	//outfile << "REMARK    1 " <<  "#   input format                    : " << me.informat << std::endl;
	outfile << "REMARK    1 " <<  "#   input files                     : " << std::endl;
	for (unsigned int i = 0; i < me.input_files.size(); i++)
	{
		outfile << "REMARK    1 " <<  "#                                     " << me.input_files[i] << std::endl;
	}
	outfile << "REMARK    1 " <<  "#   frames per thread               : " << me.num_frame_per_thread << std::endl;
	outfile << "REMARK    1 " <<  "#   number of threads               : " << me.num_thread << std::endl;
	outfile << "REMARK    1 " <<  "#   calculation threads             : " << me.num_thread_real << std::endl;
	outfile << "REMARK    1 " <<  "#   output filename prefix          : " << me.outfilename << std::endl;
	//outfile << "REMARK    1 " <<  "#   output format                   : " << me.outformat << std::endl;
	//outfile << "REMARK    1 " <<  "#   output frames per file          : " << me.output_fragment_size << std::endl;
	outfile << "REMARK    1 " <<  "#   output every n frame(s)         : " << me.output_fragment_skipframes << std::endl;
	outfile << "REMARK    1 " <<  "#   output every n picoseconds      : " << me.output_fragment_skiptime << std::endl;
	outfile << "REMARK    1 " <<  "#   initial reference coordinates   : " << me.ref_coords.x() << " " << me.ref_coords.y() << " " << me.ref_coords.z() << std::endl;
	outfile << "REMARK    1 " <<  "#   skip solvent                    : " << me.solvent_skip << std::endl;
	outfile << "REMARK    1 " <<  "#   solutes cog                     : " << std::endl;
	for (int i = 0; i < me.solute_cog_molecules.cols(); i++)
	{
		outfile << "REMARK    1 " <<  "#                                     " << me.solute_cog_molecules(0, i) << " " << me.solute_cog_molecules(1, i) << " " << me.solute_cog_molecules(2, i) << " " << me.solute_cog_molecules(3, i) << " " << me.solute_cog_molecules(4, i) << " " << me.solute_cog_molecules(5, i) << std::endl;
	}
	outfile << "REMARK    1 " <<  "#   number of solutes               : " << me.solute_count << std::endl;
	outfile << "REMARK    1 " <<  "#   solutes atom numbering          : " << std::endl;
	for (int i = 0; i < me.solute_molecules.cols(); i++)
	{
		outfile << "REMARK    1 " <<  "#                                     " << me.solute_molecules(0, i) << " " << me.solute_molecules(1, i) << " " << me.solute_molecules(2, i) << " " << me.solute_molecules(3, i) << " " << me.solute_molecules(4, i) << std::endl;
	}
	outfile << "REMARK    1 " <<  "#   number of ions                  : " << me.ion_molecules.cols() << std::endl;
	outfile << "REMARK    1 " <<  "#   ions atom numbering             : " << std::endl;
	for (int i = 0; i < me.ion_molecules.cols(); i++)
	{
		outfile << "REMARK    1 " <<  "#                                     " << me.ion_molecules(0, i) << " " << me.ion_molecules(1, i) << " " << me.ion_molecules(2, i) << " " << me.ion_molecules(3, i) << " " << me.ion_molecules(4, i) << " " << me.ion_molecules(5, i) << std::endl;
	}
	outfile << "REMARK    1 " <<  "#   skip ions                       : " << me.ions_skip << std::endl;
	outfile << "REMARK    1 " <<  "#   solvent cog images              : " << me.solvent_molecules(3, 0) << std::endl;
	outfile << "REMARK    1 " <<  "#   solvent size                    : " << me.solvent_molecules(2, 0) << std::endl;
	//outfile << "REMARK    1 " <<  "#   trc refernce file (prefix)      : " << me.trc_reference << std::endl;
	outfile << "REMARK    1 " <<  "#   comment                         : set center of geometry in viewer to (0,0,0);" << std::endl;
	outfile << "REMARK    1 " <<  "#                                     vmd: molinfo 1 set center \"{0.0 0.0 0.0}\"" << std::endl;
}

//print out the parameters gained from parsing the input file
void InputParameters::PrintGenericParameters(Structs::GenericParameters &me, std::stringstream &outfile) {
	outfile << "REMARK    1 " <<  "# input parameters defined" << std::endl;
	outfile << "REMARK    1 " <<  std::left;
	outfile << "REMARK    1 " <<  "#   number of atom records          : " << me.atomrecords << std::endl;
	//outfile << "REMARK    1 " <<  "#   distance cut-off                : " << me.distance_cut_off << std::endl;
	//outfile << "REMARK    1 " <<  "#   input format                    : " << me.informat << std::endl;
	...
	outfile << "REMARK    1 " <<  "#   output every n frame(s)         : " << me.output_fragment_skipframes << std::endl;
	outfile << "REMARK    1 " <<  "#   output every n picoseconds      : " << me.output_fragment_skiptime << std::endl;
	outfile << "REMARK    1 " <<  "#   reference coordinates           : " << me.ref_coords.x() << " " << me.ref_coords.y() << " " << me.ref_coords.z() << std::endl;
	outfile << "REMARK    1 " <<  "#   skip solvent                    : " << me.solvent_skip << std::endl;
	outfile << "REMARK    1 " <<  "#   solutes cog                     : " << std::endl;
	for (int i = 0; i < me.solute_cog_molecules.cols(); i++)
	{
	...
	outfile << "REMARK    1 " <<  "#   solvent cog images              : " << me.solvent_molecules(3, 0) << std::endl;
	outfile << "REMARK    1 " <<  "#   solvent size                    : " << me.solvent_molecules(2, 0) << std::endl;
	//outfile << "REMARK    1 " <<  "#   trc refernce file               : " << me.trc_reference << std::endl;
	outfile << "REMARK    1 " <<  "#   comment                         : set center of geometry in viewer to (0,0,0); " << std::endl;
	outfile << "REMARK    1 " <<  "#                                     vmd: molinfo 1 set center \"{0.0 0.0 0.0}\"" << std::endl;
}