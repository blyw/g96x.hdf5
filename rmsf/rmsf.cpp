...
int main(int argc, char* argv[])
{
    ...
    outfile.open((me.outfilename + ".rmsf.dat.gz").c_str(), std::ios_base::out);
    ...
    //rmsf
    me.average_coordinates.resize(3, me.atomrecords);
    me.average_coordinates.setZero();
    me.rmsf.resize(1, me.atomrecords);
    me.rmsf.setZero();
    ...
    std::thread outfileThread = std::thread([&me, &outfile, &trajs, &trajs_buffer_size, &done_calculating, &done_reading, &done_writting, &prefix, &atomNames, &atomNumbers, &chainIds, &residueNames, &residueNumbers](){
        ...
        for (int i_trajs_buffer = 0; i_trajs_buffer < trajs_buffer_size; i_trajs_buffer++)
        {
            ...
            {
                //rmsf
                me.rmsf += trajs[i_trajs_buffer].activeFrames[i_frames].rmsf_tmp;
                //std::cout << me.rmsf(0) << std::endl;
            }
            ...
            try
            {
                ...
                for (int i_threads = 0; i_threads < me.num_thread_real; i_threads++)
                {
                    myThreads[i_threads] = std::thread([i_threads, &me, &grids, &i_trajs_buffer, &trajs, &perThread, &dura]()
                    {
                        ...
                        {
#ifdef DEBUG
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates)(0, 2046) << " ";
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates)(1, 2046) << " ";
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates)(2, 2046) << std::endl;
                            std::cout << (me.average_coordinates)(0, 2046) << " ";
                            std::cout << (me.average_coordinates)(1, 2046) << " ";
                            std::cout << (me.average_coordinates)(2, 2046) << std::endl;
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates)(0, 2046) << " ";
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates)(1, 2046) << " ";
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates)(2, 2046) << std::endl;
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates).cwiseAbs2()(0, 2046) << " ";
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates).cwiseAbs2()(1, 2046) << " ";
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates).cwiseAbs2()(2, 2046) << std::endl;
                            std::cout << (trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates).cwiseAbs2().colwise().sum()(0) << std::endl;
                            std::cout << "\n" << std::endl;
#endif // DEBUG


                            //calculate RMSF
                            trajs[i_trajs_buffer].activeFrames[i_frames].rmsf_tmp =
                                ((trajs[i_trajs_buffer].activeFrames[i_frames].coordinates - me.average_coordinates).cwiseAbs2().colwise().sum());
                        }
                    });
                }
                ...
                {
#ifdef DEBUG
                    std::cout << (trajs[i_trajs_buffer].activeFrames[i_remainders].coordinates - me.average_coordinates) << std::endl;
                    std::cout << "\n" << std::endl;
#endif // DEBUG

                    //calculate RMSF
                    trajs[i_trajs_buffer].activeFrames[i_remainders].rmsf_tmp =
                        ((trajs[i_trajs_buffer].activeFrames[i_remainders].coordinates - me.average_coordinates).cwiseAbs2().colwise().sum());
                }
    ...
    //input thread
    std::thread infileThread = std::thread([&me, &frame_counter, &outfile, &trajs, &trajs_buffer_size, &done_reading, &prefix, &atomNames, &atomNumbers, &chainIds, &residueNames, &residueNumbers, &grids](){
    ...
    me.rmsf = me.rmsf / frame_counter;
    me.rmsf = me.rmsf.cwiseSqrt();
    for (int i = 0; i < me.atomrecords; i++)
    {
        ...
        outfile << "ATOM  "
            ...
            << std::setw(3) << std::right << residueNames[i] << " "
