...
#ifdef DEBUG
    std::cout << "--->HDF5: write to " << ss.str() << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
    std::cout << "--->HDF5: wrote time and steps" << std::endl;
#endif // DEBUG
    ...
    for (int i = 0; i < me->atomrecords; i++)
    {
        if (i > 99999)
        {
            for (unsigned int ii = 0; ii < chain_definition.size(); ii += 2)
            {
                ...
                {
                    atom_numbers.push_back(std::stol(framedata->prefix[i].substr(20, 5)));
                    std::string str = framedata->prefix[i].substr(13, 4);
                    str.erase(remove_if(str.begin(), str.end(), isspace), str.end());
                    atom_names.push_back(str);
                    str = framedata->prefix[i].substr(7, 4);
                    str.erase(remove_if(str.begin(), str.end(), isspace), str.end());
                    residue_names.push_back(str);
                    chains.push_back(chain_id[ii / 2]);
                    residue_numbers.push_back(std::stol(framedata->prefix[i].substr(0, 6)));
                    //Eigen takes column-major, be careful1!!
            ...
            for (unsigned int ii = 0; ii < chain_definition.size(); ii += 2)
            {
                if (i >= chain_definition[ii] && i <= chain_definition[ii + 1])
                {
                    ...
                    std::string str = framedata->prefix[i].substr(12, 4);
                    str.erase(remove_if(str.begin(), str.end(), isspace), str.end());
                    atom_names.push_back(str);
                    str = framedata->prefix[i].substr(6, 4);
                    str.erase(remove_if(str.begin(), str.end(), isspace), str.end());
                    residue_names.push_back(str);
                    chains.push_back(chain_id[ii / 2]);
...
#ifdef DEBUG
        std::cout << "--->HDF5: wrote number of atoms" << std::endl;
#endif // DEBUG

        //number of chains
        long noc = chain_definition.size() / 2;
        dims[0] = 1;
        dims[1] = 1;
        dataspace = new H5::DataSpace(2, dims);
...
#ifdef DEBUG
        std::cout << "--->HDF5: wrote number of chains" << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
        std::cout << "--->HDF5: wrote number of residues" << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
        std::cout << "--->HDF5: wrote atoms labels" << std::endl;
#endif // DEBUG

        //residue_names
        numStrings = residue_names.size();
        //std::cout << "--->HDF5: residue vector size " << numStrings << std::endl;
...
#ifdef DEBUG
        std::cout << "--->HDF5: wrote residues labels" << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
        std::cout << "--->HDF5: wrote chain labels" << std::endl;
#endif // DEBUG

        }

    //coordinates
    //Eigen takes column-major, be carefule!!!
...
#ifdef DEBUG
    std::cout << "--->HDF5: wrote coordinates" << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
    std::cout << "--->HDF5: wrote occupancies" << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
    std::cout << "--->HDF5: wrote temp factor" << std::endl;
#endif // DEBUG
...
#ifdef DEBUG
    std::cout << "--->HDF5: wrote box dimensions" << std::endl;
#endif // DEBUG
        }
